...
#include <stddef.h>
#include "server.h"
#include "SongInfoRspMsg.h"
#include "SingerInfoRspMsg.h"
...
typedef int (*MSG_HANDLE)(void *_handler, int _socket, char *_body, unsigned int _bodyLen);
...
HANDLER msgRegister[] = 
{
	{QUERY_SINGER_INFO_MSG_TYPE, Handler_HandleQuerySingerInfo, Handler_SingerInfoHandle}, 
	{QUERY_SONG_INFO_MSG_TYPE, Handler_HandleQuerySongInfo, Handler_SongInfoHandle},
...
int tcp_server_setup(char* _addr, unsigned short _port)
{
    ...
    R_ASSERT(setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(flag)) != -1, 1)
